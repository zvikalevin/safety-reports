...
async function sendNewReportEmail({ report, excelPath, photoPath, minimal = false }) {
  ...
  if (minimal) {
    ...
    // מבנה מינימלי בכוונה: טבלה אחת ברוחב קבוע, בלי מעטפת ברוחב 100%
    // ובלי רקע פרוס. מעטפות כאלה גורמות ל-Outlook להקטין את כל ההודעה
    // כשמעבירים אותה, כי הוא מחשב את ה-100% מול קנבס רחב של חלון הכתיבה.
    const html = `<table dir="rtl" align="right" role="presentation" cellpadding="0" cellspacing="0" border="0" width="520" style="width:520px; background:#ffffff; border:1px solid #d8dce1; font-family:Arial,Helvetica,sans-serif;">
