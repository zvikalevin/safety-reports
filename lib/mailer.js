...
async function sendNewReportEmail({ report, excelPath, photoPath, minimal = false }) {
  ...
  if (minimal) {
    ...
    const row = (label, value) => `
              <tr>
                <td style="padding:9px 14px; background:#f4f5f7; border-bottom:1px solid #e3e6ea; font-weight:bold; width:120px; vertical-align:top;">${label}</td>
                <td style="padding:9px 14px; border-bottom:1px solid #e3e6ea; vertical-align:top;">${value}</td>
              </tr>`;

    const html = `<div dir="rtl" style="background:#eef0f3; padding:24px 12px; font-family:Arial,Helvetica,sans-serif;">
  <table dir="rtl" role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" align="center" style="width:600px; max-width:100%; background:#ffffff; border:1px solid #d8dce1;">

    <tr>
      <td style="background:#1a2733; padding:18px 22px; color:#ffffff; font-size:19px; font-weight:bold;">
        התקבל דיווח על מפגע בטיחות
      </td>
    </tr>

    <tr>
      <td style="padding:20px 22px 4px; font-size:15px; color:#1a2733; line-height:1.6;">
    ...
    <tr>
      <td style="padding:14px 22px 0;">
        <table dir="rtl" role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="width:100%; border-collapse:collapse; font-size:14px; color:#1a2733; border:1px solid #e3e6ea;">
${row('מיקום', escapeHtml(report.location))}
${report.locationDetail ? row('פירוט מדויק', escapeHtml(report.locationDetail)) : ''}
${row('רמת דחיפות', `<span style="background:${color}; color:#ffffff; padding:3px 12px; font-weight:bold; display:inline-block;">${escapeHtml(report.urgency)}</span>`)}
...
${embedPhoto ? `
    <tr>
      <td style="padding:20px 22px 0;">
        <div style="font-size:13px; font-weight:bold; color:#6b7480; padding-bottom:8px;">תמונת המפגע</div>
        <img src="cid:hazardphoto" width="320" style="width:320px; max-width:100%; display:block; border:1px solid #d8dce1;" alt="תמונת המפגע" />
      </td>
    </tr>` : ''}

    <tr>
      <td style="padding:22px; font-size:14px; color:#1a2733; line-height:1.6;">
        נא לטפל בהתאם לרמת הדחיפות ולעדכן את סטטוס הטיפול.
      </td>
    </tr>

    <tr>
      <td style="background:#f4f5f7; border-top:1px solid #d8dce1; padding:16px 22px; font-size:13px; color:#6b7480; line-height:1.6;">
        תודה על הטיפול,<br />
