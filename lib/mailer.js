...
async function sendNewReportEmail({ report, excelPath, photoPath, minimal = false }) {
  ...
  // ---- מצב מינימלי: שורת נושא בלבד, ללא קישורים וללא צרופות ----
  if (minimal) {
    ...
    await transporter.sendMail({
      ...
      text:
        ...
        `מיקום: ${report.location}${report.locationDetail ? ' - ' + report.locationDetail : ''}\n` +
        `דחיפות: ${report.urgency}\n` +
        `מדווח: ${report.reporterName || 'אנונימי'}\n` +
        `תאריך: ${new Date(report.createdAt).toLocaleString('he-IL', { timeZone: process.env.TIMEZONE || 'Asia/Jerusalem' })}\n\n` +
        `הפרטים המלאים והתמונה זמינים בממשק הניהול.`,
    });
    return { sent: true, to: recipients, minimal: true };
  }
